...
((TOTAL++))
if curl -s -o /dev/null -w "%{http_code}" http://localhost:3001/legal-disclosure | grep -q "200"; then
...
PAGE_CONTENT=$(curl -s http://localhost:3001/legal-disclosure)
...
((TOTAL++))
HOME_CONTENT=$(curl -s http://localhost:3001/)
